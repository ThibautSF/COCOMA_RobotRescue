package stss.qlearningproject.module.complex;
...
public class SearchForFire extends Search {
	...
	public SearchForFire(AgentInfo ai, WorldInfo wi, ScenarioInfo si, ModuleManager moduleManager,
		...
		if (agentURN == AMBULANCE_TEAM) {
			this.pathPlanning = moduleManager.getModule("TestSearch.PathPlanning.Ambulance",
			...
			this.pathPlanning = moduleManager.getModule("TestSearch.PathPlanning.Fire",
					"adf.sample.module.algorithm.SamplePathPlanning");
			this.clustering = moduleManager.getModule("TestSearch.Clustering.Fire",
					"adf.sample.module.algorithm.SampleKMeans");
		} else if (agentURN == POLICE_FORCE) {
			this.pathPlanning = moduleManager.getModule("TestSearch.PathPlanning.Police",
					"adf.sample.module.algorithm.SamplePathPlanning");
	...
	public Search calc() {
		...
		for (int i = path.size() - 1; i >= 0; i--) {
			if (this.worldInfo.getEntity(path.get(i)).getStandardURN() == StandardEntityURN.ROAD) {
				this.result = path.get(i);
				break;
			}
		}
		/*
		 * if (path != null && path.size() > 2) { this.result = path.get(path.size() -
		 * 3); }else if (path != null && path.size() > 0) { this.result =
